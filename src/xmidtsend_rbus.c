...
int rbus_methodHandler(const char *methodName, cJSON *payloadJson)
{
	rbusHandle_t rbus_handle = NULL;
	rbusError_t rc;
    ...
    // 1. Combine methodName + params into a JSON string
    cJSON *reqJson = cJSON_CreateObject();
    cJSON_AddStringToObject(reqJson, "method", methodName);
    cJSON_AddItemToObject(reqJson, "params", cJSON_Duplicate(payloadJson, 1));
    char *jsonStr = cJSON_PrintUnformatted(reqJson);

    rbusValue_t propVal;
    rbusValue_Init(&propVal);
    rbusValue_SetString(propVal, jsonStr);
    rc = rbus_set(rbus_handle, "Device.X_RDK_DNSInternet.StartConnectivityCheck()", propVal, NULL);

    // Cleanup
    rbusValue_Release(propVal);
