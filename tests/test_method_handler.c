...
void test_processMethodRequest_InvalidPayload()
{
    int ret = processMethodRequest(NULL, NULL);
    assert_int_equal(ret, -1);
}

void test_processMethodRequest_JSONParseFailure()
{
    wrp_msg_t *reqMsg = calloc(1, sizeof(wrp_msg_t));
    reqMsg->u.crud.payload = strdup("{invalid json");

    expect_function_call(cJSON_Parse);
    will_return(cJSON_Parse, NULL);

    int ret = processMethodRequest(reqMsg, NULL);
    assert_int_equal(ret, -1);

    wrp_free_struct(reqMsg);
}

void test_processMethodRequest_MissingMethodField()
{
    wrp_msg_t *reqMsg = calloc(1, sizeof(wrp_msg_t));
    reqMsg->u.crud.payload = strdup("{\"notMethod\":\"value\"}");
    ...
    will_return(cJSON_GetObjectItem, NULL);
...
#ifdef ENABLE_WEBCFGBIN
        cmocka_unit_test(test_processMethodRequest_InvalidPayload),
        cmocka_unit_test(test_processMethodRequest_JSONParseFailure),
        cmocka_unit_test(test_processMethodRequest_MissingMethodField)
#endif
    };
